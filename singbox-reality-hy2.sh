...
EOF
  ...
  if grep -q ':9050' /etc/sing-box/config.json && ! netstat -tlnp 2>/dev/null | grep -q ':9050'; then
...
EOF
  ...
  update_firewall_port "" "${REALITY_PORT}"
...
while true; do
  ...
  echo -e " ${GREEN}5.${PLAIN} 管理 Tor 分流 Rule Set"
  echo -e " ${GREEN}0.${PLAIN} 退出脚本"
  echo -e "${GREEN}=================================================${PLAIN}"
  read -rp "请输入数字 [0-5]: " menu_choice
...
" 2>/dev/null | grep -q OK && {
          update_firewall_port "$CURRENT_PORT" "$NEW_PORT"
          systemctl restart sing-box
          if systemctl is-active --quiet sing-box; then
            echo -e "${GREEN}修改成功！Sing-box 已重启。${PLAIN}"
